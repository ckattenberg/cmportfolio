...
solo_john <- get_playlist_audio_features("", "37i9dQZF1DX3WXnClxyI5d")
solo_paul <- get_playlist_audio_features("", "37i9dQZF1DX2PB4Qx11qXU")
solo_george <- get_playlist_audio_features("", "37i9dQZF1DWVDVRzaLYm4M")
...
all_solo <- 
  bind_rows(
    solo_john %>% mutate(category = "John"),
    solo_paul %>% mutate(category = "Paul"),
    #solo_george %>% mutate(category = "George")
  )
...
* I labelled three outliers in the dataset: *Tomorrow Never Knows* by John has a very low valence, and *Back In The USSR* has very high energy.
...
```{r echo = FALSE}
ggplot_solo <- ggplot(data = all_solo) + 
  geom_point(mapping = aes(x = valence, y = energy, color = category), alpha = 0.7) +
  theme_classic() + 
  #scale_color_manual(values=wes_palette(n=4, name="GrandBudapest1")) + 
  scale_size(range = c(-3, 4)) + 
  theme(legend.position="none")

ggplotly(ggplot_solo)

```



### All 4 Beatles compared
...
* John Lennon has speechier songs.
* Paul's songs are often more acoustic?
...
```{r echo = FALSE}
bzt <-
  get_tidy_audio_analysis("0aym2LBJBk9DAYuHHutrIl") %>%
...
These two self-similarity matrices illustrate pitch and timbre based self similarity in the song "Hey Jude" written by Paul McCartney.
...
plot1 <- ggplot(all_songs, aes(x = track.album.release_date, y = valence, colour = category, label = track.name)) +
  geom_point() + 
  theme_light() +
