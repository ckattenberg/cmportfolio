...
ggplot(data = all_songs) + 
  geom_histogram(mapping = aes(x = valence, fill = category), position = "dodge", binwidth = 0.2) +
  theme_classic() + 
  scale_fill_manual(values=wes_palette(n=4, name="GrandBudapest1")) 

ggplot(data = all_songs) + 
  geom_histogram(mapping = aes(x = energy, fill = category), position = "dodge", binwidth = 0.2) +
  theme_classic()  + 
  scale_fill_manual(values=wes_palette(n=4, name="GrandBudapest1")) 

ggplot(data = all_songs) + 
  geom_histogram(mapping = aes(x = danceability, fill = category), position = "dodge", binwidth = 0.2) +
  theme_classic() + 
  scale_fill_manual(values=wes_palette(n=4, name="GrandBudapest1")) 

ggplot(data = all_songs) + 
  geom_histogram(mapping = aes(x = speechiness, fill = category), position = "dodge", binwidth = 0.1) +
  theme_classic() + 
  scale_fill_manual(values=wes_palette(n=4, name="GrandBudapest1")) 
```


The graphs above show that, generally speaking, John Lennon's songs don't have lower valence than Paul McCartney's songs. This is surprising because it contradicts the statement John Lennon made to Playboy Magazine. However, it does appear to be the case that Paul's songs have a higher level of danceability, according to the Spotify API. On average, John's songs are higher in energy level. This makes sense, because John said he liked to write "shouting rock 'n' roll". I also noticed that John has more "speechy" songs than Paul.
...
The differences between the Beatles' songwriting styles seems to be very small, at least in terms of valence and energy. I decided to also include some songs by George and Ringo, because it makes for a more interesting dataset. 
